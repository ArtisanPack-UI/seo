...
class InternalLinkAnalyzer implements AnalyzerContract
{
	 ...
	 * }
	 */
	public function analyze( Model $_model, string $content, ?string $_focusKeyword, ?SeoMeta $_seoMeta ): array
	{
		$issues      = [];
		$suggestions = [];
		$passed      = [];
	...
	protected function isInternalLink( string $link, string $siteHost ): bool
	{
		...
		$linkHost = preg_replace( '/^www\./i', '', $linkHost ) ?? $linkHost;
		$siteHost = preg_replace( '/^www\./i', '', $siteHost ) ?? $siteHost;
