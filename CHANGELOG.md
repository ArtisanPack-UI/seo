...
- **Laravel 13 Support**: Widened `illuminate/support` constraint to `^10.0|^11.0|^12.0|^13.0` and `orchestra/testbench` to include `^11.0`. Existing Laravel 10/11/12 users are unaffected; Laravel 13 is selectable on PHP 8.3+ via L13's own requirements (#47)
